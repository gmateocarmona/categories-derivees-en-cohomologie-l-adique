...
\vskip .3cm
Preuve~: Comme $E_m$ est un $A$--Module noethérien, l'application (6.3.2) est une bijection.
...
\vskip .3cm
Preuve~: Sous les hypothèses de (6.3.8), il résulte du lemme suivant que le morphisme (6.3.7) est un isomorphisme.
\vskip .3cm
{
...
\vskip .3cm
Preuve~: Il provient par passage à la limite projective de (6.2.2).
\vskip .3cm
...
\vskip .3cm
Preuve~: Comme pour tout entier $m$, le $A$--Module $E_m$ est localement constant de type fini, les morphismes (6.4.1.2) sont des isomorphismes (EGA $0_{III}$ 12.3.5). L'assertion en résulte sans peine. 
\vskip .3cm
...
\vskip .3cm
Preuve~: Si $f$ est cohérent, le foncteur $f_*$ commute aux limites inductives filtrantes (SGA4 VI \quad), donc les morphismes $\delta_n$ sont des isomorphismes. L'assertion en résulte aussitôt.
\vskip .3cm
...
\vskip .3cm
Preuve~: Compte tenu des définitions des seconds membres, il suffit de prouver que si $m$ et $n$ sont deux entiers, avec $m \geq n$, les morphismes canoniques
$$
...
\vskip .3cm
Preuve~: Nous montrerons seulement l'assertion non respée, l'assertion respée se prouvant de fa\c{c}on analogue. Fixons $M$. Faisant varier $N$, on obtient un morphisme de foncteurs cohomologiques 
$$
...
\vskip .3cm
Preuve~: Soit $H$ un faisceau d'ensembles tel que $L \isom B_H$. On a 
$$
...
\vskip .3cm
Preuve~: Appliquant l'assertion analogue pour les $A$--Modules aux composants de $F$, on obtient de telles suites exactes dans $\mathcal{E}(X, J)$ (resp. $\mathcal{E}(\pt, J)$).
...
\vskip .3cm
Preuve~: Pour montrer 2), on peut référer au résultat général de (Tohoku). Nous allons toutefois donner une preuve directe, qui sera utile pour montrer 1). Soient $n \geq 0$ un entier et $M$ un $A_n$--Module. On définit un $A$-faisceau noté ${}_n\widetilde{M}$ par les formules
$$
({}_n\widetilde{M})_p =
\begin{cases}
...
\vskip .3cm
Preuve~: Soit $F$ un $A$-faisceau injectif. D'après (6.6.3), il existe un monomorphisme $u: F \to P$, avec $P$ un $A$-faisceau directement strict. Comme $F$ est injectif, le morphisme $u$ admet dans $A-\fsc(X)$ une rétraction
$$
v: P \to F,
\leqno{(6.6.5)}
$$
