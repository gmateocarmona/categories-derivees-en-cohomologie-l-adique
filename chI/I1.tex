...
{
Définition {\bf 1.1}. --- \it On appelle \textup{idéotope} un triple $(X, A, J)$ formé d'un topos $X$, d'un anneau commutatif unifère $A$ et d'un idéal propre $J$ de $A$.
}
\vskip .3cm
...
J^{n + 1} F_n = 0
$$
pour tout entier $n \geq 0$. On note $\mathscr{E}(X, J)$ la sous-catégorie, abélienne, pleine de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$ engendrée par les $A$-faisceaux.
}
\vskip .3cm
Pour des raisons qui apparaîtront par la suite, la catégorie $\mathscr{E}(X, J)$ ne mérite pas le nom de catégorie des $A$-faisceaux sur $X$; c'est seulement une catégorie quotient de la précédente que nous baptiserons ainsi. Aussi, pour éviter le risque de confusion, nous arrivera-t-il, étant donnés deux $A$-faisceaux $E$ et $F$, de noter
$$
\Hom_a(E, F)
$$
($a$ pour anodin) l'ensemble des $\mathscr{E}(X, J)$-morphismes de $E$ dans $F$.

Notons pour tout objet $T$ de $X$ par $T$, ou même $T$ s'il n'y a pas de confusion possible, le topos $X/T$. Le foncteur restriction pour les faisceaux de $A$-Modules induit de fa\c{c}on évidente un foncteur restriction
$$
\mathscr{E}(X, J) \to \mathscr{E}(T, J)
$$
$$
E \mapsto E | T.
$$
...
\vskip .3cm
Preuve. Pour voir l'équivalence de (i) et (ii), il suffit d'observer que pour tout $i \in I$, le faisceau image $U_i$ de $T_i$ par le morphisme canonique $T_i \to e_X$ est tel que le morphisme restriction
$$
U_i \to T_i
$$
soit fidèle.
...
\leqno{(1.4.1)}
$$
de $\mathscr{E}(X, J)$ engendré par les $A$-faisceaux négligeables est \emph{épaisse} dans $\mathscr{E}(X, J)$.
\vskip .3cm
{
...
\vskip .3cm
Preuve. L'assertion $(i)$ est immédiate. On en déduit $(ii)$ en l'appliquant successivement à $\operatorname{Im}(v \circ u)$ et à $\Ker(v)/\operatorname{Im}(u)$. L'assertion $(iii)$ est un cas particulier de $(ii)$. Enfin $(iv)$ s'obtient en appliquant $(i)$ à $\operatorname{Im}(v-u)$.
\vskip .3cm
{
...
\vskip .3cm
Remarque {\bf 1.7.3}. En général, le préfaisceau précédent n'est pas un faisceau. Nous verrons toutefois qu'il en est ainsi, à peu de choses près, lorsque le topos $X$ est noethérien (SGA4 VI 2.11), ou lorsque $E$ est de type $J$-adique (3).
