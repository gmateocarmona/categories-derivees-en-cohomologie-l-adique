...
\vskip .3cm
{\bf 2.1}. Soit $(X, A, J)$ un idéotope. Étant donnés un objet $M$ de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$, et une application croissante $\gamma \geq \id: \mathbf{N} \to \mathbf{N}$, on définit un nouveau système projectif $c_\gamma(M)$ en posant
$$
...
$$
{\bHom}_1(\mathbf{N}^\circ, A-\Mod_X), \quad \text{comme suit}.
$$
\begin{itemize}
    \item[(i)] Ses objets sont ceux de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$. 
    \item[(ii)] Si $M$ et $M'$ sont deux objets de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$, l'ensemble des morphismes de $M$ dans $M'$ est
    $$
    \Hom_1(M, M') = \varinjlim_\gamma \Hom(c_\gamma(M), M'),
    $$
    avec la loi de composition évidente.
\end{itemize}
Il est clair qu'un morphisme de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$ définit un morphisme de $\bHom_1(\mathbf{N}^\circ, A-\Mod_X)$, d'où un foncteur ``projection''
$$
q: \bHom(\mathbf{N}^\circ, A-\Mod_X) \to {\bHom}_1(\mathbf{N}^\circ, A-\Mod_X),
$$
qui est une bijection sur les objets.
\vskip .3cm
{\bf 2.2}. Soient maintenant $M$ un objet de $\mathscr{E}(X, J)$ et $\gamma \geq \id: \mathbf{N} \to \mathbf{N}$ une application croissante. On définit un nouvel objet $\chi_\gamma(M)$ de $\mathscr{E}(X, J)$, fonctoriel en $M$, par
$$
...
$$
Ceci permet de définir une nouvelle catégorie $\mathscr{E}_1(X, J)$ comme suit.
\begin{itemize}
    \item[$(i)$] Ses objets sont ceux de $\mathscr{E}(X, J)$.
    \item[$(ii)$] Si $M$ et $N$ sont deux objets de $\mathscr{E}(X, J)$, l'ensemble des $\mathscr{E}_1(X, J)$-morphismes de $M$ dans $N$ est
    $$
...
$$
q: \mathscr{E}(X, J) \to \mathscr{E}_1(X, J).
$$

Il est clair que la catégorie $\mathscr{E}_1(X, J)$ s'identifie à la sous-catégorie pleine de $\bHom_1(\mathbf{N}^\circ, A-\Mod_X)$ engendrée pas les $A$-faisceaux, et que le diagramme
\[\begin{tikzcd}
	{\bHom(\mathbf{N}^\circ, A-\Mod_X)} && {\bHom_1(\mathbf{N}^\circ, A-\Mod_X)} \\
	{\mathscr{E}(X, J)} && {\mathscr{E}_1(X, J),}
...
Enfin, on vérifie, comme dans (SGA5 V 2.4.1), que la famille des flèches canoniques du type $\chi_\gamma(M) \to M$ permet un calcul de fractions à droite, ce qui permet d'identifier $\mathscr{E}_1(X, J)$ à la catégorie obtenue à partir de $\mathscr{E}(X, J)$ en inversant ces flèches.
\vskip .3cm
{
Lemme {\bf 2.3}. --- \it La catégorie $\Hom_1(\mathbf{N}^\circ, A-\Mod_X)$ (resp. $\mathscr{E}_1(X, J)$) est abélienne et le foncteur $q$ est exact.
}
\vskip .3cm
Preuve. Montrons tout d'abord l'assertion non respée. Il est évident que la catégorie ${\bHom}_1(\mathbf{N}^\circ, A-\Mod_X)$ est additive et que le foncteur $q$ rend inversibles toutes les flèches canoniques de la forme $c_\gamma(M) \to M$. Si $u: P \to Q$ est un élément de $\Hom_1(P, Q)$, alors $u$ est la classe d'une flèche $f: c_\gamma (P) \to Q$ et, en notant $c: c_\gamma (P) \to P$ la flèche canonique, on a donc $u = q(f) \circ q(c)^{-1}$ ; par suite, toute flèche de $\bHom_1(\mathbf{N}^\circ, A-\Mod_X)$ est isomorphe à l'image par $q$ d'une flèche de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$. Il en résulte que pour voir que $\bHom_1(\mathbf{N}^\circ, A-\Mod_X)$ est abélienne, il suffit de montrer que le foncteur $q$ est exact. Soient donc $P$ et
$$
0 \to M' \to M \to M'' \to 0
$$
respectivement un objet et une suite exacte de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$.
...
$$
\mathscr{E}_1(X, J) \to {\bHom}_1(\mathbf{N}^\circ, A-\Mod_X)
$$
est exact
\vskip .3cm
{\bf 2.4}. Soient $P$ et $Q$ deux objets de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$, $\gamma \geq \id$ une application croissante de $\mathbf{N}$ dans $\mathbf{N}$ et $f: c_\gamma(P) \to Q$ un morphisme de systèmes projectifs. Désignant pour tout $n \in \mathbf{N}$ par $\xi_n$ la classe de $f_n$ dans $\varinjlim_m \Hom(P_m, Q_n)$, il est clair que l'ensemble des $\xi_n$ $(n \in \mathbf{N})$ définit un élément de 
$$
\varprojlim_n \varinjlim_m \Hom(P_m, Q_n),
$$
qui ne dépend que de la classe de $f$ dans $\Hom_1(P, Q)$. On définit ainsi un foncteur
$$
\rho: {\bHom}_1(\mathbf{N}^\circ, A-\Mod_X) \to \Pro(A-\Mod_X),
$$
d'où un foncteur
$$
\delta: \mathscr{E}_1(X, J) \to \Pro(A-\Mod_X),
$$
obtenu par restriction de $\rho$ à $\mathscr{E}_1(X, J)$.
\vskip .3cm
{
Lemme {\bf 2.5}. --- \it Les foncteurs $\rho$ et $\delta$ ci-dessus sont des injections sur les objets et sont pleinement fidèles.
}
\vskip .3cm
Preuve. Il suffit de le voir pour $\rho$. Dans ce cas, la seule assertion non tautologique est que $\rho$ est pleinement fidèle. Soient donc $P$ et $Q$ deux objets de $\bHom(\mathbf{N}^\circ, A-\Mod_X)$, et montrons que l'application canonique
$$
\varinjlim_\gamma \Hom(c_\gamma(P), Q) \to \varprojlim_n \varinjlim_m \Hom(P_m, Q_n)
$$
est bijective.
\vskip .3cm
\emph{Elle est injective}.  Soient $\gamma$ et $\gamma'$ deux applications croissantes $\geq \id$ de $\mathbf{N}$ dans $\mathbf{N}$, et
$$
...
sont égaux.
\vskip .3cm
\emph{Elle est surjective}. Soient donnés
\begin{itemize}
...
\vskip .3cm
{\bf 2.6}. Supposons maintenant que l'objet final de $X$ soit quasicompact. Il est clair que le foncteur $q: \mathscr{E}(X, J) \to \mathscr{E}_1(X, J)$ envoie sur $O$ les $A$-faisceaux essentiellement nuls, ou, ce qui renvient au même négligeables. D'après la propriété universelle des catégories abéliennes quotients (Thèse Gabriel III 1 Cor.2), il admet donc une factorisation unique
\[\begin{tikzcd}
	{\mathscr{E}(X, J)} && {\mathscr{E}_1(X, J)}\arrow["{\overline{q}}"', from=2-2, to=1-3] \\
	& {A-\fsc(X)}
	\arrow["q", from=1-1, to=1-3]	
...
\vskip .3cm
Preuve. Tenant compte de l'interprétation de $\mathscr{E}_1(X, J)$ comme catégorie de fractions (2.2), il s'agit de voir que si $C$ est une catégorie abélienne et
$$
F: \mathscr{E}(X, J) \to C
$$
...
\vskip .3cm
Remarque {\bf 2.7.1}. Plus généralement, l'argument précédent montre, sans hypothèse sur $X$, que $\mathscr{E}_1(X, J)$ est la catégorie abélienne quotient de $\mathscr{E}(X, J)$ par la sous-catégorie, abélienne engendrée par les $A$-faisceaux essentiellement nuls. On en déduit grâce à la description des morphismes d'une catégorie abélienne quotient, que tout morphisme (resp. isomorphisme) de $A-\fsc(X)$ est localement l'image d'un morphisme de $\mathscr{E}(X, J)$.
\vskip .3cm
{
...
\vskip .3cm
Preuve. Résulte immédiatement de (2.5) et (2.7).
\vskip .3cm
Remarques {\bf 2.8.1}. La proposition précédente s'applique notamment lorsque $X$ est le topos ponctuel.
\vskip .3cm
...
\vskip .3cm
Preuve. On est ramené à voir que si $T$ est un objet noethérien de $X$ et $(T_i \to T)_{i \in I}$ est un recouvrement \emph{fini} de $T$ par des objets noethériens, alors la suite canonique
$$
...
$$
est exacte {\bf (2.7)}. Comme les produits finis sont des sommes directes et les limites inductives filtrantes de groupes abéliens sont exactes, l'assertion résulte de l'exactitude des suites canoniques 
$$
